...
{
  ros::init(argc, argv, "collect_robot");//"wheeltec_robot"); //ROS initializes and sets the node name //ROS初始化 并设置节点名称 
...
void turn_on_robot::Cmd_Vel_Callback(const geometry_msgs::Twist &twist_aux)
{
  ...
  Send_Data2.tx[0]=FRAME_HEADER; //frame head 0x7B //帧头0X7B
  ...
  Send_Data2.tx[9]=Check_Sum(9,SEND_DATA_CHECK); //For the BCC check bits, see the Check_Sum function //BCC校验位，规则参见Check_Sum函数
...
/*
Date: March 20, 2024
Functioni:jointstate callback, send commond and control robot arm 
*/void turn_on_robot::joint_states_Callback(const sensor_msgs::JointState arm_joint)
{
  static tf2_ros::TransformBroadcaster br;//发布相机坐标
  geometry_msgs::TransformStamped transformStamped;

    transformStamped.header.stamp = ros::Time::now();
    transformStamped.header.frame_id = "joint4";
    transformStamped.child_frame_id = "camera_link";
    transformStamped.transform.translation.x = 0.035;// 方向偏移3.5cm
    transformStamped.transform.translation.y = -0.06; // // 相机坐标原点在关节坐标原点的y方向上偏移6cm,反转y轴
    transformStamped.transform.translation.z = 0; // 反转z轴

    tf2::Quaternion q;
    // 先围绕z轴旋转180度
    q.setRPY(0, 0, M_PI);
    // 然后围绕新的y轴旋转180度
    q *= tf2::Quaternion(tf2::Vector3(0, 1, 0), M_PI);
...
void turn_on_robot::Publish_ImuSensor()
{
  ...
  //Imu_Data_Pub.header.stamp = ros::Time(0);
...
turn_on_robot::turn_on_robot():Sampling_Time(0),Power_voltage(0)
{
  ...
  private_nh.param<int>("joint_num", joint_num, 0);//机械臂关节起始编号
  //Odometer correction parameters
  //里程计误差修正参数
